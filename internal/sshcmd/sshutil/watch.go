...
func (ss *SSH) LoggerFileSize(host, filename string, size int) {
	...
	for {
		if <-t.C; true {
			...
			logger.DefaultLogger.Info(fmt.Sprintf("[ssh][%s]transfer total size is: %.2f%s", host, value, "MB"))
		}
	}
}
...
func (ss *SSH) IsFileExist(host, remoteFilePath string) bool {
	...
	defer func() {
		if r := recover(); r != nil {
			logger.DefaultLogger.Error(fmt.Sprintf("[ssh][%s]RemoteFileExist:%s", host, err.Error()))
		}
	}()
	if err != nil {
		panic(1)
