...
import (
	...
	"github.com/pkg/sftp"
	"github.com/wwqdrh/autobot/internal/sshcmd/md5sum"
...
func (ss *SSH) CopyForMD5(host, localFilePath, remoteFilePath, md5 string) bool {
	...
	logger.DefaultLogger.Debug("[ssh]source file md5 value is " + md5)
	ss.Copy(host, localFilePath, remoteFilePath)
	remoteMD5 := ss.Md5Sum(host, remoteFilePath)
	logger.DefaultLogger.Debug(fmt.Sprintf("[ssh]host: %s , remote md5: %s", host, remoteMD5))
	remoteMD5 = strings.TrimSpace(remoteMD5)
	md5 = strings.TrimSpace(md5)
	if remoteMD5 == md5 {
		logger.DefaultLogger.Info("[ssh]md5 validate true")
		return true
	}
	logger.DefaultLogger.Error("[ssh]md5 validate false")
	return false
}
...
func (ss *SSH) Copy(host, localFilePath, remoteFilePath string) {
	...
	defer func() {
		if r := recover(); r != nil {
			logger.DefaultLogger.Error(fmt.Sprintf("[ssh][%s]scpCopy: %s", host, err.Error()))
		}
	}()
	if err != nil {
		panic(1)
	...
	for {
		...
		logger.DefaultLogger.Info(fmt.Sprintf("[ssh][%s]transfer total size is: %.2f%s ;speed is %d%s", host, totalLength, totalUnit, speed, unit))
	}
}
...
func (ss *SSH) CopyConfigFile(host, remoteFilePath string, localFilePathOrBytes interface{}) {
	...
	defer func() {
		if r := recover(); r != nil {
			logger.DefaultLogger.Error(fmt.Sprintf("[ssh][%s]scpCopy: %s", host, err))
		}
	}()
	if err != nil {
		panic(1)
	...
	for {
		...
		logger.DefaultLogger.Info(fmt.Sprintf("[ssh][%s]transfer total size is: %d%s", host, totalMB, "MB"))
	}
}
...
func (ss *SSH) CopyLocalToRemote(host, localPath, remotePath string) {
	...
	defer func() {
		if r := recover(); r != nil {
			logger.DefaultLogger.Error(fmt.Sprintf("[ssh][%s]sshConnect err: %s", host, err.Error()))
		}
	}()
	if err != nil {
		panic(1)
	...
	defer func() {
		if r := recover(); r != nil {
			logger.DefaultLogger.Error(fmt.Sprintf("[ssh][%s]scpConnect err: %s", host, err))
		}
	}()
	if err != nil {
		panic(1)
...
func (ss *SSH) copyLocalDirToRemote(host string, sshClient *ssh.Client, sftpClient *sftp.Client, localPath, remotePath string) {
	...
	defer func() {
		if r := recover(); r != nil {
			logger.DefaultLogger.Error(fmt.Sprintf("readDir err : %s", err.Error()))
		}
	}()
	if err != nil {
		panic(1)
...
func (ss *SSH) copyLocalFileToRemote(host string, sshClient *ssh.Client, sftpClient *sftp.Client, localPath, remotePath string) {
	...
	defer func() {
		if r := recover(); r != nil {
			logger.DefaultLogger.Error(fmt.Sprintf("open file [%s] err : %s", localPath, err.Error()))
		}
	}()
	if err != nil {
		panic(1)
	...
	if err != nil {
		logger.DefaultLogger.Error(fmt.Sprintf("err: %s", err.Error()))
	}
	defer dstFile.Close()
	buf := make([]byte, 100*oneMBByte) //100mb
	total := 0
	...
	for {
		...
		totalLength, totalUnit := toSizeFromInt(total)
		logger.DefaultLogger.Debug(fmt.Sprintf("[ssh][%s]transfer local [%s] to Dst [%s] total size is: %.2f%s ;speed is %d%s", host, localPath, remotePath, totalLength, totalUnit, speed, unit))
	}
	if !ss.isCopyMd5Success(sshClient, localPath, remotePath) {
		//	logger.DefaultLogger.Debug("[ssh][%s] copy local file: %s to remote file: %s validate md5sum success", host, localPath, remotePath)
		//} else {
		logger.DefaultLogger.Error(fmt.Sprintf("[ssh][%s] copy local file: %s to remote file: %s validate md5sum failed", host, localPath, remotePath))
	}
}
...
func (ss *SSH) isCopyMd5Success(sshClient *ssh.Client, localFile, remoteFile string) bool {
	...
	defer func() {
		if r := recover(); r != nil {
			logger.DefaultLogger.Error("[ssh]Error exec command failed: " + err.Error())
		}
	}()
	if err != nil {
		panic(1)
